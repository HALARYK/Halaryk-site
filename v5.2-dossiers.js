...
function fullWarSummaryLayout(w){
  ...
  const camps=`<div class="v52-war-camps"><article><span>Coalition menée par l’Angleterre</span><strong>${esc((w.attackers||[]).join(" · "))}</strong></article><div class="v52-war-versus">VS</div><article><span>Camp ottoman</span><strong>${esc((w.defenders||[]).join(" · "))}</strong></article></div>`;
  return `<div class="v52-war-end"><div class="v52-war-result"><span>${esc(w.rpNature||"GUERRE DE COALITION")}</span><strong>${esc(w.war)}</strong></div><div class="v52-war-intro">${factCard("Objectif RP",w.warGoal,"is-accent")}${factCard("Casus belli mécanique",w.casusBelli)}${w.mechanicalWar?factCard("Nom mécanique EU4",w.mechanicalWar):""}${factCard("Période",`${longDate(w.start)} → ${longDate(w.end)}`,"is-wide")}${factCard("Batailles enregistrées",plural(w.battles,"bataille","batailles"))}</div>${camps}${sectionTitle("Pertes par camp")}<div class="v52-war-losses">${lossSide("Coalition menée par l’Angleterre",w.lossesAttackers,attackerLosses)}${lossSide("Camp ottoman",w.lossesDefenders,defenderLosses)}</div><div class="v52-war-status"><b>Issue enregistrée</b><span>${esc(w.winner)}</span></div>${w.peace?`<div class="v52-peace-box"><span>LECTURE DU RÉSULTAT</span><strong>${esc(w.peace)}</strong></div>`:""}</div>`;
}
