...
function submitGeneral(){const form=$('#event-general-form');if(form)form.requestSubmit()}
...
function eventWorkspace(){
  ...
  const eye=$(':scope > .eyebrow',panel);if(eye)eye.textContent='ADMINISTRATION · ÉVÉNEMENTS';

  const selector=card('#event-admin-select'),creator=$('#event-create-form'),general=$('#event-general-form');
  const top=document.createElement('div');top.className='v52-admin-event-head';top.innerHTML=`<button class="v52-new-event" type="button"><span>＋</span><div><b>Créer un événement</b><small>Lancer une nouvelle campagne ou un projet spécial</small></div></button><div class="v52-event-selector-slot"></div>`;
  ...
  const overviewCard=document.createElement('div');overviewCard.className='admin-card v52-overview-admin';overviewCard.innerHTML=`<div class="admin-section-title"><h2>Aperçu public</h2><p>Dates et niveaux de publication affichés sur la page de campagne. Les données récentes restent volontairement retardées.</p></div><div class="v52-overview-fields"></div><div class="admin-actions"><button class="button button-primary" type="button" data-v52-save-general>Enregistrer l’aperçu</button></div>`;
  const settingsCard=document.createElement('div');settingsCard.className='admin-card v52-settings-admin';settingsCard.innerHTML=`<div class="admin-section-title"><h2>Paramètres de publication</h2><p>Visibilité générale de l’événement et mise en avant sur l’accueil.</p></div><div class="v52-settings-fields"></div><div class="admin-actions"><button class="button button-primary" type="button" data-v52-save-general>Enregistrer les paramètres</button></div>`;
  const rules=document.createElement('div');rules.className='admin-card v52-rules-admin';rules.innerHTML=`<div class="admin-section-title"><h2>Règlement de la campagne</h2><p>Le règlement reste une sous-page propre aux Chroniques de l’Europe.</p></div><a class="button button-ghost" href="../evenements/chroniques-europe/regles/" target="_blank" rel="noopener noreferrer">Ouvrir les règles ↗</a>`;

  if(general){
    const overviewFields=[...general.children].find(x=>x.classList?.contains('admin-three-cols')&&x.querySelector('#event-world-current-input'));
    const settingFields=[...general.children].find(x=>x.classList?.contains('admin-two-cols')&&x.querySelector('#event-status-input'));
    if(overviewFields)overviewCard.querySelector('.v52-overview-fields').append(overviewFields);
    if(settingFields)settingsCard.querySelector('.v52-settings-fields').append(settingFields);
    const title=$('.admin-section-title h2',general);if(title)title.textContent='Présentation publique';
    const intro=$('.admin-section-title p',general);if(intro)intro.textContent='Nom, sous-titre, résumé et image de couverture visibles sur l’accueil et la page Événements.';
  }
  $$('[data-v52-save-general]',overviewCard).forEach(b=>b.addEventListener('click',submitGeneral));
  $$('[data-v52-save-general]',settingsCard).forEach(b=>b.addEventListener('click',submitGeneral));

  const tabs=document.createElement('nav');tabs.className='v52-event-tabs';tabs.setAttribute('aria-label','Sections de l’événement');
  const defs=[['presentation','Présentation publique'],['apercu','Aperçu'],['nations','Nations'],['chronologie','Chronologie'],['diplomatie','Diplomatie'],['medias','Médias'],['regles','Règles'],['parametres','Paramètres']];
  tabs.innerHTML=defs.map(([k,l])=>`<button type="button" data-v52-event-tab="${k}">${l}</button>`).join('');
  (creator||top).insertAdjacentElement('afterend',tabs);tabs.insertAdjacentElement('afterend',overviewCard);overviewCard.insertAdjacentElement('afterend',rules);rules.insertAdjacentElement('afterend',settingsCard);

  const snapshots=card('#event-snapshots-admin');
  const groups={
    ...
    regles:[rules],
    parametres:[settingsCard,snapshots]
  };
...
function socialPanel(){
  ...
  const panel=document.createElement('section');panel.className='hidden';panel.id='v52-social-panel';panel.innerHTML=`<p class="eyebrow">LIENS EXTERNES</p><h1>Réseaux sociaux</h1><p class="v52-admin-lead">Ces liens alimentent les icônes du site et du pied de page.</p><div class="admin-card"><div class="admin-section-title"><h2>Réseaux affichés</h2></div><div id="v52-social-list" class="v52-social-list"></div></div><form id="v52-social-form" class="admin-card"><div class="admin-section-title"><h2>Ajouter un réseau</h2></div><div class="admin-three-cols"><label>Plateforme<select id="v52-social-platform"><option value="twitch">Twitch</option><option value="discord">Discord</option><option value="instagram">Instagram</option><option value="youtube">YouTube</option><option value="tiktok">TikTok</option><option value="x">X / Twitter</option><option value="bluesky">Bluesky</option><option value="github">GitHub</option><option value="link">Autre lien</option></select></label><label>Nom affiché<input id="v52-social-label" required></label><label>Ordre<input id="v52-social-order" type="number" value="50"></label></div><label>Adresse complète<input id="v52-social-url" type="url" placeholder="https://…" required></label><label class="check-row"><input id="v52-social-enabled" type="checkbox" checked> Afficher sur le site</label><button class="button button-primary" type="submit">Ajouter</button></form>`;content.append(panel);
